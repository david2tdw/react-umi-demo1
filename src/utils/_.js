  ...
  for (const item of rows) {
    ...
    if (children && children.length > 0) {
      routes = routes.concat(
        flattenMenu(children, pathtitles.concat({ title, icon, link })),
      );
    } else if (link) {
      routes.push({
        ...item,
...
const _munesFilter = (orginalData, data, menuPermission) => {
  if (
    ...
    return {};
  const menusData = _.cloneDeep(
    Array.isArray(orginalData) ? orginalData : orginalData.rows,
  );
  ...
  const _dimensionalityReduction = (rows, names = []) => {
    rows.map(item => {
      ...
      if (children && children.length > 0) {
        ...
        console.warn(
          'The key attribute requires a valid value to be accessible',
        );
        return undefined;
      }
    });
