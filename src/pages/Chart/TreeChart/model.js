...
import { toTableData } from '@/utils/_';
...
const mergedata = (dataSource, dataset) => {
  ...
  if (key === name) {
    ...
    const childs = children.map(item =>
      item.children && item.children.length > 0 ? res : item,
    );
    return {
      name,
      children: childs,
...
export default {
    ...
    setupHistory({ dispatch, history }) {
      history.listen(({ pathname, query, state }) => {
        if (/^\/sys\/snbThree\//.test(pathname)) {
          console.log('setupHistory', pathname);
        }
      });
    },
  },
  effects: {
    *getData({ payload }, { call, put, select }) {
      ...
      const { data = {} } = yield call(fetchTree, { ...payload });
      yield put({
        type: 'updated',
        payload: {
          dataset: {
